...
class AppConfigManager(BaseManager):
    ...
    def load_plugins(self, plugins_folder: str = 'plugins', manifest_file: str = 'manifest.json'):
        """
        Load all plugins from `plugins_folder` defined in `manifest_file`
        """
        manifest_data = read_json(f'{plugins_folder}/{manifest_file}')
        ...
        for plugin in manifest_data.get('plugins'):
            ...
            plugin_config_files = [glob.glob(f'{plugins_folder}/{plugin_name}/configs/*.json')
                                   for _ in os.listdir(plugins_folder)]
